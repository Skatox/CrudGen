...
class Pages {
    ...
    public function getPageNotcompleted($appid,$page_id="") {
        ...
        $sql = "SELECT page_id FROM appgen.pages WHERE completed=false AND app_id={$appid}";
        if($page_id!="") $sql = $sql." AND page_id={$page_id}";
    ...
    public function saveTable($page_id) {
        ...
        $sql = "INSERT INTO appgen.page_tables (table_name, pages_page_id) "
    ...
    public function insert($app_id) {
        ...
        $sql = "INSERT INTO appgen.pages (page_filename, page_title, operation,page_text,completed,app_id)"
     ...
     *
     * @param $appid Application's id from database, so it will load a page from selected application
     * @param $page_id the desired page's database id
     */
    public function load($appid,$page_id) {
        global $misc;

        $driver = $misc->getDatabaseAccessor("phppgadmin");
        $sql = "SELECT page_filename, page_title,descr, completed,date_created,operation,in_main_menu,page_text FROM appgen.pages "
            ."WHERE page_id={$page_id} AND app_id={$appid}";
        ...
        switch($this->operation) {
            case "b": $this->operation="browse";break;
            case "i": $this->operation="insert";break;
            case "u": $this->operation="update";break;
            case "d": $this->operation="delete";break;
        }

        //Retreives table's name
        $sql = "SELECT page_tables_id, table_name FROM appgen.page_tables WHERE pages_page_id={$page_id}";
        $rs= $driver->selectSet($sql);
    ...
    public function update() {
        ...
        // Creates a new database access object.
        $driver = $misc->getDatabaseAccessor("phppgadmin");
        $sql = "UPDATE appgen.pages SET page_filename='{$this->page_filename}',page_title='{$this->page_title}',page_text='{$this->page_text}',descr='{$this->descr}',"
    ...
    public function delete() {
        ...
        // Creates a new database access object.
        $driver = $misc->getDatabaseAccessor("phppgadmin");
        $sql = "DELETE FROM appgen.pages WHERE page_id={$this->page_id}";
        return $driver->execute($sql);
    }
    /**
    ...
    public function validate(){
        ...
        if(empty($this->page_title)) return $lang['strnopagetitle'];
        if(empty($this->page_filename)) return $lang['strnopagefilename'];
        
         //Checks if page's filename doesn't have extension
        if (substr($this->page_filename,-4)!=".php")
                return $lang['strerrnoextension'];
     ...
     */
    public static function getPagesDB() {
        global $misc, $lang;
        $driver = $misc->getDatabaseAccessor("phppgadmin");
        $sql = "SELECT page_id,page_title,date_created,page_filename,"
                . "CASE WHEN operation='d' THEN '{$lang['strdelete']}' WHEN operation='i' THEN '{$lang['strinsert']}' "
                . "WHEN operation='u' THEN '{$lang['strupdate']}' ELSE '{$lang['strselect']}' END AS operation, "
                . "CASE WHEN completed='f' THEN '{$lang['strno']}' ELSE '{$lang['stryes']}' END AS completed "
                . "FROM crudgen.pages WHERE app_id={$this->app_id} ORDER BY operation";
        $rs = $driver->selectSet($sql);
        return $rs;
    }
}
